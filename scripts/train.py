...
def preprocess(
  ...
  val_dataset = (
    load_dataset(val_data_path, feature_index) if val_data_path else None
  )
...
def pred(
  scores: jax.Array, rows: jax.Array, cols: jax.Array, N: int
) -> jax.Array:
  ...
  r: jax.Array = (
    2 * jax.ops.segment_sum(scores.take(cols), rows, N) - scores.sum()
  )
  return r > 0
...
def fit(
  ...
  def output_progress(t: int) -> None:
    ...
    with open(log_filename, "a") as f:
      pred_train = pred(
        scores, dataset_train.X_rows, dataset_train.X_cols, N_train
      )
