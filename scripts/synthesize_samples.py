...
def verify_bug_reproduction(target: IntentTarget, parser: budoux.Parser) -> bool:
...
def align_to_base_parser_splits(
  candidates: list[str], target: IntentTarget, parser: budoux.Parser
) -> list[str]:
...
def extract_intent_target(
  issue_text: str, client: genai.Client, model_name: str = "gemini-3.1-flash-lite"
) -> IntentTarget:
...
"""
  config = genai.types.GenerateContentConfig(
    response_mime_type="application/json", response_schema=IntentTarget, temperature=0.1
  )
...
def run_agentic_synthesis_pipeline(
  ...
  parser: budoux.Parser | None = None,
) -> list[str]:
  ...
  aligned_candidates = align_to_base_parser_splits(raw_candidates, target, parser)
...
def build_parser() -> argparse.ArgumentParser:
  ...
  p.add_argument(
    "--lang", type=str, default="ja", help="Target model language (default: ja)"
  )
  p.add_argument(
    "--num-samples", "-n", type=int, default=30, help="Initial generation count"
  )
