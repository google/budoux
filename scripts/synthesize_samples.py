...
def fetch_github_issue_context(
  issue_id: str, requests_mod: Any = requests
) -> str:
...
def verify_bug_reproduction(
  target: IntentTarget, parser: budoux.Parser
) -> bool:
...
def run_agentic_synthesis_pipeline(
  ...
  aligned_candidates = align_to_base_parser_splits(
    raw_candidates, target, parser
  )
